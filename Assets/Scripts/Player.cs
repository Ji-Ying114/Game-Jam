...
public class Player : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (_warningLevel>= _maxWarningLevel)
        {
            ...
            foreach(var enemy in _enemys)
            {
                enemy.GetComponent<Enemy>().ChasePlayer();
            }
        }
        SetLifeBar((_warningLevel / _maxWarningLevel) % 1);
        if (isDay)
        {
            SetColddownBarDay(_colddown);
            if (_colddown <= 0)
            {
                SwichtoNight();
            }
